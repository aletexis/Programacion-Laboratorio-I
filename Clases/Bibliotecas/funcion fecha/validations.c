...
int validations_getDate(eDate date, int minYear, int maxYear)
{
    ...
    if((date.day < '0' || date.day > '9') && (date.month < '0' || date.month > '9') && (date.year < '0' || date.year > '9'))
    {
        ret = 0;
    }
    else
    {
        ...
        if((date.month != 2) || (!isLeapYear (date.year)))	//si no es febrero o no es bisiesto
        {
        ...
        else if((date.month == 2) && (isLeapYear (date.year))) //si es febrero y es bisiesto
        {
            if (date.day < 1 || date.day > monthDays[2]+1) //pongo como dia maximo 28 + 1
            {
                ret = 0;
            }
        }
