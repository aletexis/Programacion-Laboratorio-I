...
int validations_getConfirmChar(char* result, char* message, char* messageError, int tries)
{
    ...
    if(result != NULL && message != NULL && messageError != NULL && tries >= 0)
    {
        do
        {
            if(!getString(message,messageError,1,3,&tries,bufferChar))
            {
                if(isValidConfirmChar(bufferChar[0]) == 1)
                {
                    *result = bufferChar[0];
                    ret = 0;
                    break;
...
int isValidConfirmChar(char character)
{
    ...
    if(character != 'S' && character != 's' && character != 'N' && character != 'n')
    {
        ret = 0;
    }
...
int validations_getPath(char* input, char* message, char* messageError, int minSize, int maxSize, int tries)
{
    ...
    if(input != NULL && message != NULL && messageError != NULL && minSize < maxSize && tries >= 0)
    {
        do
        {
            if(!getString(message,messageError,minSize,maxSize,&tries,bufferString))
            {
                if(isValidPath(bufferString, sizeof(bufferString)) == 1)
                {
                    strncpy(input,bufferString,maxSize);
                    ret = 0;
                    break;
...
int isValidPath(char* string, int lenght)
{
    ...
    if(string != NULL && lenght > 0)
    {
        ...
        for(int i=0; i<lenght && string[i] != '\0'; i++)
        {
            if((string[i] < '0' || string[i] > '9') && (string[i] < 'A' || string[i] > 'Z') && (string[i] < 'a' || string[i] > 'z') && string[i] != '.' && string[i] != '"' )//fijarse el espacio
            {
                ret = 0;
                break;
            }
